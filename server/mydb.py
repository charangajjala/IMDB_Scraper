...
class MyDB:
    ...
    def __init__(self):
        db = self.give_db()
        self._basic_details_collection = self.give_collection(db, MyDB._BasicDetials_COLLECTION_NAME)
        self._reviews_collection = self.give_collection(db, MyDB._Reviews_COLLECTION_NAME)
    ...
    def handle_basic_details(self,title,basic_details=None):
        details = self._basic_details_collection.find_one({'title': title})
